  ...
  const initializedCountries = [].concat(...countries.map((country) => {
    ...
    country[6] &&
      country[6].map((areaCode) => {
        const areaItem = Object.assign({}, countryItem);
...
function extendUserContent(userContent, contentItemIndex, extendingObject, firstExtension = false) {
...
function extendRawCountries(countries, userContent) {
  ...
  return countries.map(o => {
    ...
    if (userContentIndex === -1)
      return o; // if iso2 not in userContent, return source country object
    const userContentCountry = userContent[userContentIndex];
    if (userContentCountry[1])
      o[4] = userContentCountry[1]; // mask
    if (userContentCountry[3])
      o[5] = userContentCountry[3]; // priority
    if (userContentCountry[2])
      o[6] = userContentCountry[2]; // areaCodes
    return o;
  });
}
class CountryData {
  constructor(enableAreaCodes, enableTerritories, regions, onlyCountries, preferredCountries, excludeCountries, preserveOrder, masks, priority, areaCodes, localization, prefix, defaultMask, alwaysDefaultMask) {
    ...
    if (enableTerritories) {
      let [initializedTerritories] = initCountries(rawTerritories$1, enableAreaCodes, prefix, defaultMask, alwaysDefaultMask);
    ...
    this.onlyCountries = this.localizeCountries(this.excludeCountries(this.getFilteredCountryList(onlyCountries, initializedCountries, preserveOrder.includes('onlyCountries')), excludeCountries), localization, preserveOrder.includes('onlyCountries'));
    this.preferredCountries = preferredCountries.length === 0 ? [] :
      this.localizeCountries(this.getFilteredCountryList(preferredCountries, initializedCountries, preserveOrder.includes('preferredCountries')), localization, preserveOrder.includes('preferredCountries'));
    // apply filters to hiddenAreaCodes
    this.hiddenAreaCodes = this.excludeCountries(this.getFilteredCountryList(onlyCountries, hiddenAreaCodes, false), excludeCountries);
  }
}
